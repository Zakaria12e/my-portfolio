...
export function ModernHeader() {
  ...
  scrolled
    ? "mx-6 md:mx-20 lg:mx-32 mt-3 rounded-2xl bg-background/80 backdrop-blur-md border shadow-lg px-4 py-3"
    : "px-0 py-6"
}`}
      >
        <div className="container flex items-center justify-between">
          ...
          >
            <img src="/moon-purple.png" alt="Moon" className="w-12 h-12 hidden dark:block" />
            <img src="/moon-dark.png" alt="Moon" className="w-12 h-12 block dark:hidden" />
          </motion.div>
          ...
          <nav className="hidden md:flex absolute left-1/2 transform -translate-x-1/2 items-center gap-10">
            {navItems.map((item) => (
              ...
              >
                {item.name}
                <span className="absolute left-0 -bottom-1 h-[2px] w-0 bg-primary transition-all duration-300 hover:w-full" />
              </motion.a>
            ))}
          </nav>
